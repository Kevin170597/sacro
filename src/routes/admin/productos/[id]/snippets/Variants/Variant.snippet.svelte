...
<div
    class="px-8 py-4 flex items-center rounded-t-lg {variantDropped ===
    variantIndex
...
>
    ...
    >
        <span class="text-[14px]">{variant.name || "Sin nombre"}</span>
        {#if variantIndex === 0}
            <span
...
{#if variantDropped === variantIndex}
    <div class="flex px-4 pb-2">
        <VariantNameInput bind:value={variant.name} errors={errors && errors.name || null} />
        <VariantColorInput bind:value={variant.hexColor} />
    </div>
    <VariantImages bind:variantImages={variant.images} errors={errors && errors.images || null} />
    <VariantSize bind:sizes={variant.size} errors={errors && errors.size || null} />
{/if}
